...
namespace XHX.View
{
    public partial class Area : BaseForm
    {
        ...
        {
            Workbook workbook = msExcelUtil.OpenExcelByMSExcel(btnModule.Text);
            Worksheet worksheet_FengMian = workbook.Worksheets["AreaUserIdPassword"] as Worksheet;
            for (int i = 2; i < 90; i++)
            {
                string areaCode = msExcelUtil.GetCellValue(worksheet_FengMian, "A", i);
                if (!string.IsNullOrEmpty(areaCode))
                {
                    string userId = msExcelUtil.GetCellValue(worksheet_FengMian, "B", i);
                    string password = msExcelUtil.GetCellValue(worksheet_FengMian, "C", i);
                    webService.UpDateAreaPassword(areaCode,userId, password);
                }
            }
            CommonHandler.ShowMessage(MessageType.Information, "上传完毕");
        }
